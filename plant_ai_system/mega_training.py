"""
Mega Training Script - Tích hợp tất cả datasets để tạo hệ thống Plant AI mạnh mẽ nhất
"""
...
class MegaDatasetLoader(Dataset):
    """Mega dataset loader cho tất cả datasets"""
    ...
    def _normalize_class_name(self, class_name, dataset_name):
        ...
        elif 'common_rust' in class_name.lower():
            return 'Common_Rust'
...
class MegaPlantTrainer:
    """Mega trainer cho tất cả datasets"""
    ...
    def __init__(self, config):
        ...
        print(f"Mega Plant Trainer initialized on {self.device}")
    ...
    def load_datasets(self):
        ...
        data_config = {
            ...
            'durian_diseases': {
                ...
                'type': 'structured',
                'category': 'tropical_fruit_diseases'
            },
            'augmented_plant_diseases': {
                'path': 'data/archive/New Plant Diseases Dataset(Augmented)/New Plant Diseases Dataset(Augmented)',
                'type': 'structured',
                'category': 'augmented_plant_diseases'
            },
            'crop_pest_diseases': {
        ...
        # Tạo dataset loaders
        train_dataset = MegaDatasetLoader(data_config, mode='train')
        val_dataset = MegaDatasetLoader(data_config, mode='val')
    ...
    def create_model(self):
        ...
        # Sử dụng EfficientNet-B3 cho hiệu suất tốt hơn
        try:
    ...
    def train_model(self, train_loader, val_loader, epochs=30):
        """Training model với advanced techniques"""
        if self.model is None:
            self.create_model()
        
        # Advanced optimizer và scheduler
        ...
        print(f"Starting mega training for {epochs} epochs...")
        ...
        for epoch in range(epochs):
            ...
            if val_acc > best_val_acc:
                ...
                self.save_model(f"mega_plant_model_epoch_{epoch+1}.pth", epoch, val_loss)
        ...
        with open('models/mega_training_history.json', 'w') as f:
            json.dump(history, f, indent=2)
        
        print(f"\nMega training completed! Best validation accuracy: {best_val_acc:.2f}%")
        return best_val_acc
...
def main():
    """Main mega training function"""
    print("MEGA PLANT AI TRAINING - Integrate all datasets")
    print("=" * 70)
    ...
    # Initialize trainer
    trainer = MegaPlantTrainer(config)
    
    # Load datasets
    print("\n1. Loading mega datasets...")
    train_loader, val_loader = trainer.load_datasets()
    
    # Create model
    print("\n2. Creating mega model...")
    trainer.create_model()
    
    # Train model
    print("\n3. Starting mega training...")
    best_acc = trainer.train_model(train_loader, val_loader, epochs=30)
    
    # Test model
    print("\n4. Testing mega model...")
    test_acc = trainer.test_model(val_loader)
    
    print(f"\n[SUCCESS] MEGA TRAINING COMPLETED!")
    print(f"Best validation accuracy: {best_acc:.2f}%")
    print(f"Test accuracy: {test_acc:.2f}%")
    print(f"Total classes: {len(trainer.classes)}")
    print(f"Model saved to: models/mega_plant_model_epoch_*.pth")
    print(f"History saved to: models/mega_training_history.json")
